...
SERVER_RELEASE_NAME=library-server
CLIENT_RELEASE_NAME=library-client
...
serverLatestTag=$(curl -s "https://registry.hub.docker.com/v2/repositories/$DOCKERHUB_USERNAME/library-app-server/tags" | jq -r '.results | map(select(.name != "latest")) | sort_by(.last_updated) | reverse | .[0].name')
clientLatestTag=$(curl -s "https://registry.hub.docker.com/v2/repositories/$DOCKERHUB_USERNAME/library-app-client/tags" | jq -r '.results | map(select(.name != "latest")) | sort_by(.last_updated) | reverse | .[0].name')
...
echo "Deploying server: $DOCKERHUB_USERNAME/library-app-server:$serverLatestTag using spring-app chart $springAppChartVersion"
...
# can transiently fill with both photos and their base64 copies (see the
# ENTRYPOINT in server/Dockerfile - keep the two in step).
helm upgrade $SERVER_RELEASE_NAME mucsi96/spring-app \
    --install \
    --version $springAppChartVersion \
    --set image=$DOCKERHUB_USERNAME/library-app-server:$serverLatestTag \
    --set entryPoint=web \
    --set host=$HOSTNAME \
    --set basePath=/api \
...
echo "Deploying client: $DOCKERHUB_USERNAME/library-app-client:$clientLatestTag using client-app chart $clientAppChartVersion"
